...
export default function SpotifyNowPlaying() {
  ...
  return (
    <div className="relative group h-full">
      <div
        className={`absolute inset-0 ${result?.isPlaying ? "bg-primary" : "bg-muted"} rounded-sm blur-2xl opacity-10 group-hover:opacity-20 transition-opacity duration-1000`}
      ></div>
        ...
        <div className="p-5 flex flex-col items-center justify-center gap-5 h-full">
              ...
              <div className="w-24 h-24 md:w-32 md:h-32 relative">
                {result ? (
                  <motion.img
                    ...
                    animate={
                      result.isPlaying ? { scale: [1, 1.01, 1] } : { scale: 1 }
                    }
                    transition={{
                      duration: 8,
                      repeat: Infinity,
                ...
                <div
                  className={`absolute -bottom-2 -right-2 ${result?.isPlaying ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"} rounded-none p-1 shadow-2xl border border-border z-10`}
                >
          ...
          <div className="flex-1 min-w-0 w-full flex flex-col items-center justify-center text-center">
            <div className="flex items-center justify-center gap-2 mb-2">
              <span
                className={`text-[10px] font-mono font-bold uppercase tracking-[0.2em] ${result?.isPlaying ? "text-primary" : "text-muted-foreground"}`}
              >
                {loading
                  ? "WAIT"
                  : result?.isPlaying
            ...
            {loading ? (
              ...
              >
                <h3
                  className={`font-mono font-bold text-xs md:text-sm truncate transition-colors uppercase tracking-tight ${result.isPlaying ? "text-foreground group-hover/text:text-primary" : "text-muted-foreground"}`}
                >
                  {result.title}
                </h3>
              ...
              <div className="opacity-30">
                <h3 className="font-mono font-bold text-[10px] uppercase tracking-widest text-muted-foreground">
                  Signal Lost
                </h3>
                <p className="text-[9px] font-mono uppercase mt-1 text-muted-foreground">
                  Standby
                </p>
              </div>
            )}
