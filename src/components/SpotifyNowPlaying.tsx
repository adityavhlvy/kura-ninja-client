    ...
    return (
        <div className="relative group h-full">
            ...
            <div className="relative card bg-base-100/80 backdrop-blur-md shadow-xl border border-base-content/10 overflow-hidden w-full h-full hover:scale-[1.02] transition-transform duration-300">
                <div className="card-body p-6 flex flex-col items-center justify-center gap-6 h-full">
                    <div className="relative shrink-0">
                        {loading ? (
                            <div className="w-32 h-32 md:w-48 md:h-48 bg-base-300 rounded-xl animate-pulse"></div>
                        ) : (
                            <div className="w-32 h-32 md:w-48 md:h-48 relative">
                                {(result.isPlaying || result.isRecent) ? (
                                    <motion.img
                                        ...
                                        alt={result.title}
                                        className={`w-full h-full rounded-2xl object-cover shadow-2xl ${!result.isPlaying && 'grayscale-[50%]'} ${result.isPlaying && 'animate-spin-slow'}`}
                                    ...
                                    <div className="w-full h-full bg-base-200 rounded-2xl flex items-center justify-center text-base-content/30">
                                        <SlSocialSpotify size={64} />
                                    </div>
                                )}
                                <div className={`absolute -bottom-3 -right-3 ${result.isPlaying ? 'bg-[#1DB954]' : 'bg-base-content'} text-base-100 rounded-full p-2 shadow-lg border-4 border-base-100`}>
                    ...
                    <div className="flex-1 min-w-0 w-full flex flex-col items-center justify-center text-center">
                        ...
                        {loading ? (
                            ...
                            >
                                <h3 className={`font-bold text-lg md:text-xl truncate transition-colors ${result.isPlaying ? 'group-hover/text:text-[#1DB954]' : 'text-base-content/70'}`}>
                                    {result.title}
                                </h3>
                                <p className="text-sm md:text-base text-base-content/60 truncate font-medium mt-1">
                                    {result.artist}
                                </p>
                            </a>
                        ) : (
                            <div>
                                <h3 className="font-bold text-lg text-base-content/50">Not Playing</h3>
                                <p className="text-sm text-base-content/40">Spotify</p>
                            </div>
                        )}
                    </div>
                </div>
